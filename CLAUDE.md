...
- In docstrings, inline code/monospace must use double backticks: ````foo```` not `` `foo` ``. Exception: RST cross-reference roles (e.g. ``:class:`~freshpointparser.models.types.FieldDiff```) use single backticks by Sphinx convention — do not change these.
